...
fix.50bin = read_delim("Data/ET_fix.txt", delim="\t")
```

- `fix.window` data: fixation proportion in a single window from -800 ms to 0 ms relative to the target word onset
- `fix.50bin` data: fixation proportion in every 50 ms bin from -1000 ms to 1000 ms relative to the target word onset

We will first look at the `fix.window` data.   
...
You can view the entire data by clicking `fix.window` in the Environment.  

The dataset `fix.window` contains the following information:
...
fix.window = fix.window %>% mutate_at(vars(Item), as.character) %>% mutate_at(vars(Item), as.numeric)
summary(fix.window)
```
...
```{r eval=T, echo=T, message=F}
summary(fix.window$Condition)
```

The labels are ordered alphabetically.  
Let's change this to `Targ`, `Eng`, `Jap` and `Unr`.
```{r eval=T, echo=T, message=F}
fix.window = fix.window %>% mutate(Condition = fct_relevel(Condition,c('Targ','Eng','Jap','Unr')))
```
...
```{r eval=T, echo=T, message=F}
fix.window[fix.window$Subject %in% c('p2','j2','j4'),]
```
...
fix.window[fix.window$Subject=='j2'|fix.window$Item==10,] # Q3.1

fix.window[fix.window$Trial==5&fix.window$Lang=='L1',] # Q3.2
...
fix.window %>% filter(Subject %in% c('j2','p2','p9'), Condition == 'Targ') # Q4.1

fix.window %>% filter(Subject %in% c('j2','p2','p9'), Condition == 'Targ') %>% select(-Condition) # Q4.2
...
```{r eval=T, echo=T, message=F}
summarySE(fix.window, measurevar = 'FixP', groupvars = 'Condition')
```


To compute mean, SD, SE, and 95% CI for each condition and for each `Lang` group, you can run this:
```{r eval=T, echo=T, message=F}
summarySE(fix.window, measurevar = 'FixP', groupvars = c('Lang','Condition'))
```


Let's save the second summary as `fix.window.summary`:
```{r eval=T, echo=T, message=F}
fix.window.summary = summarySE(fix.window, measurevar = 'FixP', groupvars = c('Lang','Condition'))
```
...
ggplot(fix.window.summary[fix.window.summary$Lang=='L1',], aes(x = Condition, y = FixP, fill = Condition)) +
  ggtitle("L1 speakers") +
  geom_bar(stat = "identity") +
...
Now, let's look at the `fix.50bin` data.  
